...
import net.mcreator.ragnoroktaleofthevikings.item.NifilheimDimenstionItem;
import net.mcreator.ragnoroktaleofthevikings.block.EternalPermafrostBlock;
...
public class NifilheimDimenstionDimension extends RagnorokTaleOfTheVikingsModElements.ModElement {
	@ObjectHolder("ragnorok_tale_of_the_vikings:nifilheim_dimenstion_portal")
	public static final CustomPortalBlock portal = null;
	public NifilheimDimenstionDimension(RagnorokTaleOfTheVikingsModElements instance) {
		super(instance, 9);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("ragnorok_tale_of_the_vikings:nifilheim_dimenstion"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("nifilheim_dimenstion_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("nifilheim_dimenstion_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("nifilheim_dimenstion_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new NifilheimDimenstionItem().setRegistryName("nifilheim_dimenstion"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("ragnorok_tale_of_the_vikings:nifilheim_dimenstion"))) {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == EternalPermafrostBlock.block.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? EternalPermafrostBlock.block.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, EternalPermafrostBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
